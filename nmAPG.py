...
# [3] https://github.com/johertrich/LearnedRegularizers
...
def nmAPG(
    ...
    if progress:
        loss_all = [f(x,y).item()]
        psnr_all = [compare_psnr(img.detach().cpu().numpy(), x.detach().cpu().numpy())]
    # Main loop
    for i in range(max_iter):
        ...
        if i > 0:
            ...
            res[idx] = torch.norm(nabla(x[idx],y[idx]), p=2, dim=(1, 2, 3)) 
        assert not torch.any(
            torch.isnan(res)
        ), "Numerical errors! Some values became NaN!"
        condition = res >= tol
        ...
        grad_old.copy_(grad)
        if progress:
            loss_all.append(f(x,y).item())
            psnr_all.append(compare_psnr(img.detach().cpu().numpy(), x.detach().cpu().numpy()))
    if verbose and (torch.max(res) >= tol):
        print(f"max iter reached, tol {torch.max(res).item():.6f}")
    converged = res < tol
    return (x, L, i+1, converged, loss_all, psnr_all, torch.max(res)) if progress else (x, L, i+1, converged, torch.max(res))
...
def reconstruct_nmAPG(
    ...
    def energy(val, y_in):
        with torch.no_grad():
            fun = data_fidelity(val, y_in, physics) + lamda * regularizer.g(val).squeeze(1)
        if detach_grads:
            fun = fun.detach()
    ...
    # example energies
    if progress:
        rec, L, steps, converged, loss_all, psnr_all, res = nmAPG(
            x0=x,
            y=y,
            img=x_gt,
            max_iter=max_iter,
        ...
        stats = dict(L=L.detach(), steps=steps, converged=converged, loss_all=loss_all, psnr_all=psnr_all, res=res)
    else:
        rec, L, steps, converged, res = nmAPG(
            x0=x,
            y=y,
            img=x_gt,
            max_iter=max_iter,
        ...
        stats = dict(L=L.detach(), steps=steps, converged=converged, res=res)
