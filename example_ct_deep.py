...
from models import deeper_ICNNPrior
...
# Load regularizer
regularizer = deeper_ICNNPrior(1,32,5,512,2,device)
ckp = torch.load('weights/reg_icnn_ct_deep.pt')
...
def M1(u_in):
    return [regularizer.icnn.W1(u_in[1]), u_in[2]]

def M2(u_in):
    return [regularizer.icnn.W2(u_in[2]), u_in[3]]

def M3(u_in):
    return [regularizer.icnn.WA(u_in[3])]


x = fbp.detach().clone().to(device)
z1 = F.leaky_relu(regularizer.icnn.wx(x), 0.2).detach().to(device)
z2 = F.leaky_relu(regularizer.icnn.wz[0](z1), 0.2).to(device)
z3 = F.leaky_relu(regularizer.icnn.wz[1](z2), 0.2).to(device)
u = [x, z1, z2, z3]

def MAT(v_in):
    return [AT(v_in[0]), torch.zeros_like(z1), torch.zeros_like(z2), torch.zeros_like(z3)]

def M0T(v_in):
    return [regularizer.icnn.W0T(v_in[0]), v_in[1], torch.zeros_like(z2), torch.zeros_like(z3)]

def M1T(v_in):
    return [torch.zeros_like(x), regularizer.icnn.W1T(v_in[0]), v_in[1], torch.zeros_like(z3)]

def M2T(v_in):
    return [torch.zeros_like(x), torch.zeros_like(z1), regularizer.icnn.W2T(v_in[0]), v_in[1]]

def M3T(v_in):
    return [torch.zeros_like(x), torch.zeros_like(z1), torch.zeros_like(z2), regularizer.icnn.WAT(v_in[0])]

M0norm = power_LL(M0, M0T, u)
M1norm = power_LL(M1, M1T, u)
M2norm = power_LL(M2, M2T, u)
M3norm = power_LL(M3, M3T, u)
MAnorm = power_LL(MA, MAT, u)
W0norm = power(regularizer.icnn.W0, regularizer.icnn.W0T, u[0])
W1norm = power(regularizer.icnn.W1, regularizer.icnn.W1T, u[1])
W2norm = power(regularizer.icnn.W2, regularizer.icnn.W2T, u[2])
WAnorm = power(regularizer.icnn.WA, regularizer.icnn.WAT, z3)
...
def M1_new(u_in):
    return [regularizer.icnn.W1(u_in[1]) / M1norm, u_in[2] / M1norm]

def M2_new(u_in):
    return [regularizer.icnn.W2(u_in[2]) / M2norm, u_in[3] / M2norm]

def M3_new(u_in):
    return [regularizer.icnn.WA(u_in[3]) / M3norm]

def MT_x(v_in):
    return AT(v_in[0][0]) / MAnorm + regularizer.icnn.W0T(v_in[1][0]) / M0norm

def MT_z1(v_in):
    return v_in[1][1] / M0norm + regularizer.icnn.W1T(v_in[2][0]) / M1norm

def MT_z2(v_in):
    return v_in[2][1] / M1norm + regularizer.icnn.W2T(v_in[3][0]) / M2norm

def MT_z3(v_in):
    return v_in[3][1] / M2norm + regularizer.icnn.WAT(v_in[4][0]) / M3norm
...
b1 = regularizer.icnn.wz[0].bias.data.view(1, regularizer.icnn.n_filters, 1, 1)
b2 = regularizer.icnn.wz[1].bias.data.view(1, regularizer.icnn.n_filters, 1, 1)
bf1 = regularizer.icnn.fc1.bias.data
a_weight = reg_param * regularizer.icnn.fc2.weight.data
cmin = 0 * a_weight
cmax = a_weight * M3norm
...
data_pd_all = []
reg_pd_all = []
ca, c1, c2, c3, c4 = 500, 100, 5, 0.5, 0.1
with torch.no_grad():
    sigmaa = ca / (opnorm / MAnorm) ** 2
    sigma0 = c1 / (W0norm / M0norm) ** 2
    sigma1 = c2 / (W1norm / M1norm) ** 2
    sigma2 = c3 / (W2norm / M2norm) ** 2
    sigma3 = c4 / (WAnorm / M3norm) ** 2

    taux = 1 / (ca + c1)
    tauz1 = 1 / (sigma0 / M0norm ** 2 + c2)
    tauz2 = 1 / (sigma1 / M1norm ** 2 + c3)
    tauz3 = 1 / (sigma2 / M2norm ** 2 + c4)

    x = fbp
    z1 = F.leaky_relu(regularizer.icnn.wx(x), 0.2).to(device)
    z2 = F.leaky_relu(regularizer.icnn.wz[0](z1), 0.2).to(device)
    z3 = F.leaky_relu(regularizer.icnn.wz[1](z2), 0.2).to(device)

    va = [1 - sino / (A(x) + r)]
    v1 = [torch.zeros_like(z1), torch.zeros_like(z1)]
    v2 = [torch.zeros_like(z2), torch.zeros_like(z2)]
    v3 = [torch.zeros_like(z3), torch.zeros_like(z3)]
    v_init = reg_param * regularizer.icnn.fc2.weight.data.clone()
    v_init[regularizer.icnn.WA(z3) + bf1 <= 0] = 0
    v4 = [v_init]
    v = [va, v1, v2, v3, v4]
    xold, z1old, z2old, z3old = x, z1, z2, z3
    ubar = [2 * x - xold, 2 * z1 - z1old, 2 * z2 - z2old, 2 * z3 - z3old]
    ...
    for i in range(max_iter):    
        ...
        vin2 = [a + sigma1 * b for a, b in zip(v[2], M1_new(ubar))]
        projv2 = proj_Lrelu([M1norm * vin2[0] / sigma1 + b1, M1norm * vin2[1] / sigma1], 0.2)
        v[2][0] = vin2[0] - sigma1 * (projv2[0] - b1) / M1norm
        v[2][1] = vin2[1] - sigma1 * projv2[1] / M1norm
        vin3 = [a + sigma2 * b for a, b in zip(v[3], M2_new(ubar))]
        projv3 = proj_Lrelu([M2norm * vin3[0] / sigma2 + b2, M2norm * vin3[1] / sigma2], 0.2)
        v[3][0] = vin3[0] - sigma2 * (projv3[0] - b2) / M2norm
        v[3][1] = vin3[1] - sigma2 * projv3[1] / M2norm

        v[4][0] = torch.clamp((M3norm * (v[4][0] + sigma3 * M3_new(ubar)[0]) + sigma3 * bf1) / M3norm, cmin, cmax)
        ...
        psnr_pd_all_gt.append(compare_psnr(phantom.detach().cpu().numpy(), x.detach().cpu().numpy()))
